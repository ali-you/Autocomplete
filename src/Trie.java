...
public class Trie  {
    ...
    public String toString() {
        ...
        printWords(root, wordArray, 0, sb);
        if (sb.toString().length() == 0)
            return "";
        return sb.toString().substring(1);
    }

    private void printWords(TrieNode root, char[] wordArray, int pos, StringBuilder sb) {
        if(root == null)
            return;
        if(root.isEnd)  {
            sb.append("\n");
        ...
        for (int i = 0; i < 26; i++) {
            if (root.nodes[i] != null) {
                ...
                printWords(root.nodes[i], wordArray, pos+1, sb);
            }
        }
    }
    ...
    private boolean compareTrie(Trie p, Trie q) {
        ...
        if (strs1.length!= strs2.length)
            return false;
        for (String s: strs1) {
            TrieNode node1 = p.search(s);
            TrieNode node2 = q.search(s);
